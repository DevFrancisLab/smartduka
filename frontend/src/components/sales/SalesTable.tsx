...
const SalesTable = ({ newSales = [] }: SalesTableProps) => {
  ...
  return (
    <div className="bg-card border border-border rounded-lg shadow-sm overflow-hidden flex flex-col h-full">
      ...
      <div className="overflow-y-auto flex-1">
        ...
        {/* Render new sales submitted by the user below demo data */}
        {newSales.map((s, i) => {
          const time = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
          const id = `new-${i}-${Date.now()}`;
          return (
            <div
              key={id}
              className={`px-4 py-3 border-b border-border last:border-b-0 hover:bg-muted-foreground/5 transition-colors cursor-pointer ${
                (sales.length + i) % 2 === 0 ? 'bg-muted-foreground/2' : ''
              }`}
            >
              <div className="md:hidden space-y-2">
                <div className="flex items-start justify-between">
                ...
                <div className="grid grid-cols-3 gap-2 text-xs">
                  <div>
                    <p className="text-muted-foreground">Qty</p>
                    <p className="font-semibold text-foreground">{s.quantity}</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground">Price</p>
                    <p className="font-semibold text-foreground">Ksh {s.pricePerUnit}</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground">Total</p>
                    <p className="font-semibold text-gradient-gold">Ksh {s.total}</p>
                  </div>
                </div>
              ...
              <div className="hidden md:grid md:grid-cols-5 gap-4 items-center text-sm">
                ...
                <div className="text-right text-foreground font-medium">{s.quantity}</div>
                <div className="text-right text-foreground">Ksh {s.pricePerUnit}</div>
                <div className="text-right font-semibold text-gradient-gold">Ksh {s.total}</div>
                <div className="text-right text-muted-foreground flex items-center justify-end gap-2">
                  <span>{time}</span>
                  <ChevronRight className="w-4 h-4 text-muted-foreground/50" />
                </div>
