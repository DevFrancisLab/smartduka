...
const ExpensesTable = ({ newExpenses = [] }: ExpensesTableProps) => {
  ...
  return (
    <div className="bg-card border border-border rounded-lg shadow-sm overflow-hidden flex flex-col h-full">
      ...
      <div className="overflow-y-auto flex-1">
        ...
        {/* Render new expenses submitted by user below demo entries */}
        {newExpenses.map((e, i) => {
          ...
          return (
            <div
              key={id}
              className={`px-4 py-3 border-b border-border last:border-b-0 hover:bg-muted-foreground/5 transition-colors cursor-pointer ${
                (expenses.length + i) % 2 === 0 ? 'bg-muted-foreground/2' : ''
              }`}
            >
              <div className="md:hidden space-y-2">
                <div className="flex items-start justify-between">
                  <p className="text-sm font-medium text-foreground flex-1 pr-2">{e.type}</p>
                  <ChevronRight className="w-4 h-4 text-muted-foreground/50 flex-shrink-0 mt-0.5" />
                </div>
                ...
                <div className="grid grid-cols-3 gap-2 text-xs">
                  <div>
                    <p className="text-muted-foreground">Amount</p>
                    <p className="font-semibold text-destructive">Ksh {e.amount}</p>
                  </div>
                  <div className="col-span-2">
                    <p className="text-muted-foreground">Notes</p>
                    <p className="font-semibold text-foreground truncate">{e.notes}</p>
                  </div>
                </div>
              ...
              <div className="hidden md:grid md:grid-cols-4 gap-4 items-center text-sm">
                ...
                <div className="text-right font-semibold text-destructive">Ksh {e.amount}</div>
                <div className="text-foreground truncate">{e.notes}</div>
                <div className="text-right text-muted-foreground flex items-center justify-end gap-2">
                  <span>{time}</span>
                  <ChevronRight className="w-4 h-4 text-muted-foreground/50" />
                </div>
